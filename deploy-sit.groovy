...
node (){
    ...
    dir (env.BUILD_ID){
        ...
        stage ("deploy"){
            ...
            ocpDeploy session: "${namespace}-ekyc-web-${BUILD_NUMBER}", namespace: namespace, serviceName: serviceName, ocpUrl: ocpUrl, ocpCredential: ocpCredential, dockerRegistry: dockerRegistry, image: "${dockerRegistry}/${namespace}/ekyc-web:${version}", template: "deploy/template.yml", variable: "deploy/sit.env", parameters: "domain=apps.dev.corp.btpn.co.id namespace=${namespace} version=${version}"
        }
    }
}