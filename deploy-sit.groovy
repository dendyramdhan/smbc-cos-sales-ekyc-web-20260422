...
node (){
    ...
    dir (env.BUILD_ID){
        ...
        def namespace = "customer-administrative-platform-sit"
        ...
        stage ("deploy"){
            ...
            ocpDeploy session: "${namespace}-ekyc-web-${BUILD_NUMBER}", namespace: namespace, serviceName: serviceName, ocpUrl: ocpUrl, ocpCredential: ocpCredential, dockerRegistry: dockerRegistry, image: "${dockerNexus}/${nsService}:${version}", template: "deploy/template.yml", variable: "deploy/sit.env", parameters: "domain=${domain} namespace=${namespace} version=${version} replica=${replica}"
        }
    }
}